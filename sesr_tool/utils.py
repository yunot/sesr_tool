...
#Conver YCbCr to RGB
def ycbcr_to_rgb(ycbcr: tf.Tensor):
    ycbcr -= tf.constant([[[16., 128., 128.]]])
    m = tf.constant([[65.481, 128.553, 24.966],
                      [-37.797, -74.203, 112.0],
                      [112.0, -93.786, -18.214]])
    trans_m = tf.linalg.matrix_transpose(m)
    inv_m = tf.linalg.inv(trans_m)
    rgb = tf.linalg.matmul(ycbcr, inv_m)
    return rgb * 255
